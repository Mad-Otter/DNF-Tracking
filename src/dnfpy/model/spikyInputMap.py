...
previous_input = np.zeros((201, 201))
...
def spiking(*varlist):
    ...
    for i in range(len(res)):
        for j in range(len(res[0])):
            if res[i, j] > np.random.random():
                res[i, j] = 1
            else:
                res[i, j] = 0
    return res

def differential_spiking(*varlist):
    ...
    for i in range(len(res)):
        for j in range(len(res[0])):
            if res[i, j] - previous_input[i, j] > np.random.random():
                res[i, j] = 1
            else:
                res[i, j] = 0
...
class SpikyInputMap(FuncWithoutKeywords):
    ...
    def __init__(self,name,size,dt=0.1,wrap=True,distr_dt=0.1,noise_dt=0.1,noiseI=0.01,
        ...
        super(SpikyInputMap,self).__init__(spiking,name,size,dt=dt,
